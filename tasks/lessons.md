...
> **[2026-05-23] Lesson:** `expo-file-system` `cacheDirectory` and `documentDirectory` are both `null` in Expo Go on some Android versions. Any feature that writes files to the local filesystem (data export, PDF generation) will fail silently or throw at the point of `FileSystem.writeAsStringAsync`. Guard against this by checking `cacheDir` before writing and throwing a tagged error (`code: 'EXPO_GO_FS_UNAVAILABLE'`) so the calling screen can show a friendly message instead of a generic error. These features require an EAS build for full functionality.

> **[2026-05-28] Lesson:** Android OAuth custom URI scheme redirect may show a browser interception modal when a third party browser is set as default — this is expected behaviour and works correctly. The user taps 'Open in another app' and lands in LanaLibre. This cannot be fixed without changing the OAuth flow entirely.
